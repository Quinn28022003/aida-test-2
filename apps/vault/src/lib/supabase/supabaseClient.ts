...
/**
 * Browser-safe Supabase client for Vault.
...
export function createSupabaseClient(options?: CreateSupabaseClientOptions): SupabaseClient {
    const supabaseUrl = options?.supabaseUrl ?? getVaultPublicEnv().NEXT_PUBLIC_SUPABASE_URL;
    const supabaseAnonKey =
        options?.supabaseAnonKey ?? getVaultPublicEnv().NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
