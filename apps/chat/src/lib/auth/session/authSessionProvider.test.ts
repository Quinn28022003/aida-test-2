...
vi.mock('../identityRedirect', () => ({
    buildIdentityAuthRedirectUrl: vi.fn(() => 'http://localhost:3006/reset-password/update'),
    buildIdentityLoginRedirectFromLocation: vi.fn(() => 'http://localhost:3006/login'),
...
describe('AuthSessionProvider', () => {
    ...
    it('clears auth state when onAuthStateChange reports sign-out', async () => {
    ...
    it('invalidates me query cache on SIGNED_IN', async () => {
        let authListener: ((event: string, session: { access_token: string } | null) => void) | undefined;
        ...
        await waitFor(() => {
            expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: profileQueryKey });
        });
    });
});
