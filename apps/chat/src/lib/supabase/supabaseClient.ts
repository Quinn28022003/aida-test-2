...
/**
 * Browser-safe Supabase client for Chat.
...
export function createSupabaseClient(options?: CreateSupabaseClientOptions): SupabaseClient {
    const supabaseUrl = options?.supabaseUrl ?? getChatPublicEnv().NEXT_PUBLIC_SUPABASE_URL;
    const supabaseAnonKey =
        options?.supabaseAnonKey ?? getChatPublicEnv().NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
